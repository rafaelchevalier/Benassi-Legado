...
if(!function_exists('verifica_situacao_carrinho')){
	function verifica_situacao_carrinho($num_serie){
		$sql = mysql_query("
			SELECT * FROM mov_carrinho
			WHERE num_serie = '$num_serie'
...
if(!function_exists('baixa_situacao_carrinho')){// Desativa Carrinho de flores
	function baixa_situacao_carrinho($id){
		require "conexao.php";
		$sql = (" UPDATE mov_carrinho SET situacao='0' where id='$id'");
		mysql_query($sql);
	}
}

if(!function_exists('ativa_situacao_carrinho')){// Desativa Carrinho de flores
	function ativa_situacao_carrinho($id){
		require "conexao.php";
		$sql = (" UPDATE mov_carrinho SET situacao='1' where id='$id'");
		mysql_query($sql);
	}
}
...
// ******** funcoes para cont_caixas *************

//busca contagem real do último lançamento
if (!function_exists('busca_ultimo_lancamento_cont_caixa')) {
	function busca_ultimo_lancamento_cont_caixa($id_mercado,$cod_caixa){
		$data_atual =  date("d/m/Y");
		$data = converte_data($data_atual);
		
		$sql = mysql_query("
			SELECT * FROM cont_caixas
		...
		");
		while($linha = mysql_fetch_array($sql)){
			$saldo = $linha['saldo_oficial'];
			//$saldo = $linha['saldo_oficial'] + $linha['recebido'] -  $linha['enviado'];//Calcula campos de Recebimento e Saída antes de retornar valor.
		}
		return ($saldo);
	}
}
if (!function_exists('bloqueia_lancamento_no_mesmo_dia_cont_caixa')) {
	function bloqueia_lancamento_no_mesmo_dia_cont_caixa($id_mercado,$data,$cod_caixa){
		//$data_atual =  date("d/m/Y");
...
// ******** Fim funcoes para cont_caixas *************
//*************** ======== Funções globais ========= ******************/
if (!function_exists('select_selecionado')) {
	function select_selecionado($id){
		if(!empty($id)){
...
/**************************************************************************************************************************
							Inicio Função busca banco Autocompletar
**************************************************************************************************************************/

/**************************************************************************************************************************
							Fim Função busca banco Autocompletar
**************************************************************************************************************************/
