...
if($funcao == "abre_chamado"){//Adiciona novo chamado
	...
	echo "<script type=\"text/javascript\">	
		...
		language='javascript'>history.back(-1)
	 </script>";
...
$id = $_GET['id'];
fecha_chamado(utf8_decode($_REQUEST['descricao']),utf8_decode($_REQUEST['nome_tecnico']),utf8_decode($_REQUEST['detalhe']),utf8_decode($_REQUEST['solucao']),utf8_decode($_REQUEST['detalhe_erro']));
	echo"
	<META HTTP-EQUIV=REFRESH content='0; URL=../chamado.php?pg=consulta'>
	<script type=\"text/javascript\">	
	alert(\" Chamado fechado com sucesso.\");
	</script>";
}

function fecha_chamado($descricao,$nome_tecnico,$detalhe,$solucao,$detalhe_erro){
		$id = $_GET['id'];
	//Verifica se a situação do Chamado é de status (Pendente, Aberto, Em Andamento ou Respondido) caso não seja ele fecha o chamado.
	if($solucao == 'Pendente' || $solucao == 'ABERTO' || $solucao == 'Em Andamento' || $solucao == 'RESPONDIDO'){
		//$detalhe_novo = $detalhe." /p".converte_data($data_atual)." - ".$hora_atual;
		$sql = (" UPDATE chamado SET situacao='ABERTO', descricao='$descricao',detalhe='$detalhe',atendido_por='$nome_tecnico',solucao='$solucao', hora_atendimento=now(), data_atendimento=now() where id='$id'");
	}
	else{
		$sql = (" UPDATE chamado SET situacao='FECHADO',detalhe='$detalhe',atendido_por='$nome_tecnico',solucao='$solucao', hora_atendimento=now(), data_atendimento=now() where id='$id'");
	}
